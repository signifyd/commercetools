...
public class AcceptCommand extends DecisionCommand {

    public AcceptCommand(ConfigReader configReader,
                         CommercetoolsClient commercetoolsClient,
                         Order order,
                         DecisionResponse decisionResponse) {
        super(configReader, commercetoolsClient, order, decisionResponse, Action.ACCEPT);
    }

    @Override
    public List<OrderUpdateAction> generateOrderActions() {
        return super.generateDefaultOrderActions(configReader.getDecisionActions(order.getCountry()).getAccept());
    }

    @Override
    public Order executeOrderActions() {
        return super.executeDefaultOrderActions(configReader.getDecisionActions(order.getCountry()).getAccept());
    }
}
